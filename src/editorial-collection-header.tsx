...
export function EditorialCollectionHeader({ heading, countLabel, view, onViewChange }: EditorialCollectionHeaderProps) {
  return <header className="editorial-collection-toolbar"><>{heading ? <p className="editorial-collection-heading">{heading}</p> : <span />}</><div className="editorial-collection-actions">{countLabel && <span className="editorial-collection-count">{countLabel}</span>}<div className="editorial-collection-toggle" role="group" aria-label="콘텐츠 보기 방식"><button type="button" className={view === "list" ? "is-active" : ""} aria-pressed={view === "list"} onClick={() => onViewChange("list")}><ListIcon /><span className="editorial-visually-hidden">리스트 보기</span></button><button type="button" className={view === "grid" ? "is-active" : ""} aria-pressed={view === "grid"} onClick={() => onViewChange("grid")}><GridIcon /><span className="editorial-visually-hidden">카드 보기</span></button></div></div></header>;
}
