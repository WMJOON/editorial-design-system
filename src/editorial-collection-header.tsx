...
function ViewButton({ active, label, value, onPress, children }: { active: boolean; label: string; value: EditorialCollectionView; onPress: () => void; children: ReactNode }) {
  ...
  return <button
    ...
    aria-pressed={active}
    data-analytics-event="change_content_view"
    data-analytics-view-name={value}
...
export function EditorialCollectionHeader({ heading, countLabel, view, onViewChange }: EditorialCollectionHeaderProps) {
  return <header className="editorial-collection-toolbar"><>{heading ? <p className="editorial-collection-heading">{heading}</p> : <span />}</><div className="editorial-collection-actions">{countLabel && <span className="editorial-collection-count">{countLabel}</span>}<div className="editorial-collection-toggle" role="group" aria-label="콘텐츠 보기 방식"><ViewButton active={view === "list"} label="리스트 보기" value="list" onPress={() => onViewChange("list")}><ListIcon /></ViewButton><ViewButton active={view === "grid"} label="카드 보기" value="grid" onPress={() => onViewChange("grid")}><GridIcon /></ViewButton></div></div></header>;
}
