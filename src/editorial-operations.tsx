...
export function EditorialEditorShell({ title, children, mode = "edit", actions }: { title: string; children: ReactNode; mode?: "preview" | "edit"; actions?: ReactNode }) {
  return <section className="editorial-ops-editor" aria-label="Markdown 편집 패턴"><header className="editorial-ops-editor-header"><div><h2 className="editorial-title-2"><EditorialWordWrap>{title}</EditorialWordWrap></h2></div><button className="editorial-ops-button" type="button">닫기</button></header><div className="editorial-ops-editor-toolbar"><div><button className={`editorial-ops-button${mode === "preview" ? " is-active" : ""}`} type="button">미리보기</button><button className={`editorial-ops-button${mode === "edit" ? " is-active" : ""}`} type="button">Markdown 편집</button></div>{actions ?? <button className="editorial-ops-button primary" type="button">로컬에 저장</button>}</div>{children}</section>;
}
